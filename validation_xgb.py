...
def run_train_test(ds_name, metric, xgb_params):
    ...
    x_test, _, line_id, is_test, _ = load_data(f'{path}/test.csv', mode='test')
    y_test = load_test_label(f'{path}/test-target.csv')

    x_train_proc, col_stats = preprocessing(x_train, y_train)
    x_test_proc, _ = preprocessing(x_test, 0, col_stats)
